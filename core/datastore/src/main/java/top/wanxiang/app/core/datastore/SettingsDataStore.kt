...
class SettingsDataStore @Inject constructor(
    ...
    private data class StoredEnvironmentVariable(
        val metadata: top.wanxiang.app.core.model.EnvironmentVariable,
    ...
    val effectiveExecutionMode: Flow<top.wanxiang.app.core.model.ExecutionMode> = context.settingsDataStore.data.map { preferences ->
        top.wanxiang.app.core.model.ExecutionMode.fromId(preferences[executionModeKey] ?: top.wanxiang.app.core.model.ExecutionMode.PROOT.id)
    }

    /** 用户最后一次成功选择的模式；缺少新键时从旧 execution_mode 无损迁移。 */
    val preferredExecutionMode: Flow<top.wanxiang.app.core.model.ExecutionMode> = context.settingsDataStore.data.map { preferences ->
        top.wanxiang.app.core.model.ExecutionMode.fromId(
            preferences[preferredExecutionModeKey]
                ?: preferences[executionModeKey]
                ?: top.wanxiang.app.core.model.ExecutionMode.PROOT.id,
        )
    }

    /** 兼容旧消费者：executionMode 始终表示实际生效模式。 */
    val executionMode: Flow<top.wanxiang.app.core.model.ExecutionMode> = effectiveExecutionMode

    suspend fun setExecutionMode(mode: top.wanxiang.app.core.model.ExecutionMode) {
        context.settingsDataStore.edit { preferences ->
    ...
    suspend fun setPreferredExecutionMode(mode: top.wanxiang.app.core.model.ExecutionMode) {
        context.settingsDataStore.edit { it[preferredExecutionModeKey] = mode.id }
    }

    suspend fun setEffectiveExecutionMode(mode: top.wanxiang.app.core.model.ExecutionMode) {
        context.settingsDataStore.edit { it[executionModeKey] = mode.id }
    }

    suspend fun setExecutionModes(
        preferred: top.wanxiang.app.core.model.ExecutionMode,
        effective: top.wanxiang.app.core.model.ExecutionMode,
    ) {
        context.settingsDataStore.edit {
    ...
    /** 最近一次成功拉取的云端配置（JSON 序列化的 Map<String,String>），离线兜底用。 */
    val cloudConfigJson: Flow<String> = context.settingsDataStore.data.map { it[cloudConfigJsonKey].orEmpty() }

    /** 最近一次云端配置拉取成功的毫秒时间戳。 */
    val cloudConfigFetchedAt: Flow<Long> = context.settingsDataStore.data.map { it[cloudConfigFetchedAtKey] ?: 0L }

    suspend fun setCloudConfig(json: String) {
        context.settingsDataStore.edit { preferences ->
            preferences[cloudConfigJsonKey] = json
    ...
    suspend fun setLastSeenAnnouncementId(id: Long) {
        context.settingsDataStore.edit { preferences ->
            preferences[lastSeenAnnouncementIdKey] = id
        }
    }
    ...
    val allPlugins: Flow<List<top.wanxiang.app.core.model.AgentPlugin>> = context.settingsDataStore.data.map { prefs ->
        val defaults = top.wanxiang.app.core.model.BuiltinPlugins.presets
            .filter { it.isEnabled }
            .mapTo(mutableSetOf()) { it.id }
        val enabledIds = prefs[enabledPluginsKey] ?: defaults
        top.wanxiang.app.core.model.BuiltinPlugins.presets.map { plugin -> plugin.copy(isEnabled = plugin.id in enabledIds) }
    }
