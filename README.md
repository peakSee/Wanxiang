...
1. **安装运行**：在 ARM64、Android 10+（推荐 Android 12+）设备上安装 [最新 Release APK](https://github.com/peakSee/Wanxiang/releases)。
...
构建产物位于：`app/build/outputs/apk/debug/wanxiang-v0.11.0-debug.apk`

> 📦 **WanXiangDev 双包共存构建**：在 CI 或本地设置环境变量 `$env:WANXIANG_DEV_BUILD="1"`，可产出包名为 `top.wanxiang.app.dev` 的独立预览版，与正式版完全共存互不干扰。
...
└── feature/              # Jetpack Compose 业务特性层
    ├── components/      # 万象 M3 Expressive 设计规范、Lucide 图标、Spotlight 引导、通用卡片
