# 万象（WanXiang）自定义迭代与 WanXiangDev 构建模块 (`feature:custom_iteration`)

本模块为 **万象 · WanXiang** 量身打造的标准、无降级自迭代系统。让用户在手机上使用万象的自主 AI Agent 进行万象自身的二次开发、测试、云端 CI 构建，并向开源社区贡献代码。
...
├── build.gradle.kts                        # Gradle 多模块依赖配置（对齐万象规范）
...
└── src/
    ...
    │   └── java/top/peakSee/Wanxiang/
    │       ├── iteration/engine/
    │       │   ├── CustomIterationBootstrap.kt   # 自举引擎（工作区初始化/引导 Prompt 生成）
    │       │   └── WanXiangDevBuildCoordinator.kt   # CI 调度与 APK 下载校验协调器
    │       └── ui/iteration/
...
2. **专属 Agent Skill 约束 (`SKILL.md`)**：
   * 指导 AI Agent 严格遵守万象的多模块 Kotlin DSL 规范、Material3 Compose 规范、协程设计及凭证安全规则。
