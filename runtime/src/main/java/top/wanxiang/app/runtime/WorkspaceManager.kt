...
class WorkspaceManager @Inject constructor(
    ...
    private val buildScriptRepository: dagger.Lazy<top.wanxiang.app.core.database.BuildScriptRepository>? = null,
) {
    constructor(
    ...
    private suspend fun executeTemplateHook(
        ...
        val hookFile = File(projectDir, ".wanxiang-template-$stage-${UUID.randomUUID()}.sh")
        try {
            ...
            val result = linuxRuntime.get().execute(
                ShellCommand(
                    ...
                    environment = values.mapKeys { (name, _) -> "WANXIANG_VAR_${name.uppercase()}" } +
                        ("WANXIANG_PROJECT_DIR" to linuxPathFor(projectDir)),
    ...
    companion object {
        private const val UNLINKED_MARKER = ".wanxiang-unlinked-project"
        private const val PROJECT_METADATA_FILE = ".wanxiang-project.properties"
        private const val IMPORT_STAGING_DIRECTORY = ".wanxiang-import-staging"
