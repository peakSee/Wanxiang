...
import top.wanxiang.app.core.model.CpuArch
...
import top.wanxiang.app.runtime.proot.QemuCompatibilityLayout
import top.wanxiang.app.runtime.proot.syncGuestGroups
import top.wanxiang.app.runtime.bridge.HostBridge
import top.wanxiang.app.runtime.pty.PtyManager
import top.wanxiang.app.runtime.proot.ProotInstaller
import top.wanxiang.app.runtime.rootfs.RootfsInstaller
import top.wanxiang.app.runtime.terminal.terminalBanner
...
class LinuxRuntimeImpl @Inject constructor(
    ...
    private val settingsDataStore: top.wanxiang.app.core.datastore.RuntimePreferences,
    private val storageMountBindingRepository: top.wanxiang.app.core.database.StorageMountBindingRepository,
    ...
    private val _installedDistros = MutableStateFlow<List<top.wanxiang.app.core.model.InstalledDistro>>(emptyList())
    override val installedDistros: StateFlow<List<top.wanxiang.app.core.model.InstalledDistro>> = _installedDistros.asStateFlow()
    ...
    override fun refreshInstalledDistros() {
        ...
        val list = ids.map { id ->
            ...
            top.wanxiang.app.core.model.InstalledDistro(
                id = id,
    ...
    override suspend fun updateRootfs(distroId: String?): AppResult<Unit> = initializeMutex.withLock {
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                processRegistry.stopAll()
                updateInitializing("更新 RootFS", 0.05f, "保留 /root 和 /opt/wanxiang 用户数据")
    ...
    private suspend fun resolveExecutionLayout(
        ...
        if (!useQemuCompatibility) {
            return ExecutionLayout(pathManager.rootfsDir(distroId), null, pathManager.wanxiangRootDir(distroId))
        }
        ...
        check(QemuCompatibilityLayout.isReady(wanxiangRoot)) {
            "QEMU 兼容环境未就绪，请先安装并验证 qemu-x86-64-compat 插件"
        }
        return ExecutionLayout(
            rootfsDir = QemuCompatibilityLayout.guestRootfs(wanxiangRoot),
            emulatorBinary = QemuCompatibilityLayout.qemuBinary(wanxiangRoot),
            optDir = wanxiangRoot,
        )
    }
    ...
    override suspend fun startSession(config: SessionConfig, distroId: String?): LinuxSession {
        ...
        val markerFile = File(pathManager.wanxiangRootDir(safeDistro), ".pty-$markerId")
        val markerPath = "/opt/wanxiang/.pty-$markerId"
        val mounts = storageMounts()
        ...
        if (config.showBanner) {
            runCatching {
                File(pathManager.wanxiangRootDir(safeDistro), "motd").writeText(terminalBanner())
            }.onFailure {
            ...
            val session = if (ptyManager.nativeAvailable) {
                ptyManager.openNative(
                    command = prootCommandBuilder.buildInteractive(
                        ...
                        workspaceDir = pathManager.workspaceDir,
                        homeDir = pathManager.homeDir(safeDistro),
                        optDir = pathManager.wanxiangRootDir(safeDistro),
                        tmpDir = pathManager.tmpDir,
                        attachmentsDir = pathManager.attachmentsDir,
                        config = config,
                        nativePty = true,
        ...
        runCatching {
            shellExecutor.execute(
                command = prootCommandBuilder.build(
                    ...
                    optDir = pathManager.wanxiangRootDir(distroId),
                    tmpDir = pathManager.tmpDir,
                    attachmentsDir = pathManager.attachmentsDir,
    ...
    private fun configureRootfs(distroId: String = "ubuntu") {
        ...
        etcDir.mkdirs()
        File(etcDir, "wanxiang-runtime").writeText("wanxiang-runtime=0.3.0\n")
        File(rootfs, "opt/wanxiang").mkdirs()
    ...
    private fun configureDpkgNoDoc(distroId: String = "ubuntu") {
        ...
        dpkgCfgDir.mkdirs()
        File(dpkgCfgDir, "01_wanxiang_nodoc").writeText(
            """
    ...
    private fun configureAptSettings(distroId: String = "ubuntu") {
        ...
        hookDir.mkdirs()
        File(hookDir, "99wanxiang-apt-config").writeText(
            """
    ...
    private fun renameToDisabled(file: File) {
        val disabled = File(file.parentFile, "${file.name}.wanxiang-disabled")
    ...
    private fun installPerlFixScript(distroId: String = "ubuntu") {
        ...
        binDir.mkdirs()
        val script = File(binDir, "wanxiang-fix-perl")
        script.writeText(
            """
            #!/bin/sh
            set -e
    ...
    private fun installAptStripSetuidHook(distroId: String = "ubuntu") {
        ...
        File(hookDir, "99wanxiang-strip-setuid").writeText(
            "// PRoot 沙箱：setuid 不生效，保留会导致 dpkg 升级卡死（见 wanxiang-runtime 文档）。\n" +
    ...
    private fun configureEnvironment(distroId: String = "ubuntu") {
        ...
        profileDir.mkdirs()
        File(profileDir, "wanxiang-env.sh").writeText(
            """
            export LANG=C.UTF-8
            export PATH=/root/.local/bin:/opt/wanxiang/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
            export HOME=/root
            # HostBridge — 沙箱通过 localhost HTTP 桥接触发宿主侧操作
            export WANXIANG_BRIDGE_URL="http://127.0.0.1:7980"
            export WANXIANG_BRIDGE_PORT=7980
            # Android 二进制参考路径（用 wanxiang-android-exec 包装器执行）
     ...
     * 安装宿主桥接沙箱端脚本与 API 密钥。
     *
     * 写入三个文件到 /opt/wanxiang/（bind-mounted 到沙箱）：
     * - bin/wanxiang-host        — 桥接 CLI（install-apk / shell / health）
     * - bin/wanxiang-android-exec — Android 二进制执行包装器（设置正确的 linker 环境）
     * - .bridge-key           — API 认证密钥
     */
    private fun installHostBridgeScripts(distroId: String) {
        val binDir = pathManager.wanxiangBinDir(distroId)
        binDir.mkdirs()
        val rootDir = pathManager.wanxiangRootDir(distroId)

        // wanxiang-host — 宿主桥接 CLI
        val hostScript = File(binDir, "wanxiang-host")
        hostScript.writeText(WANXIANG_HOST_SCRIPT)
        runCatching { android.system.Os.chmod(hostScript.absolutePath, 0x1ED) } // 0755

        // wanxiang-android-exec — Android 二进制执行包装器
        val androidExecScript = File(binDir, "wanxiang-android-exec")
        androidExecScript.writeText(WANXIANG_ANDROID_EXEC_SCRIPT)
    ...
    companion object {
        ...
        val WANXIANG_HOST_SCRIPT = listOf(
            ...
            "KEY_FILE=\"/opt/wanxiang/.bridge-key\"",
            "",
            "if [ -f \"\${KEY_FILE}\" ]; then",
            ...
            "case \"\$1\" in",
            "  install-apk)",
            "    [ -z \"\$2\" ] && { echo 'Usage: wanxiang-host install-apk <path>' >&2; exit 1; }",
            ...
            "    ;;",
            "  shell)",
            "    [ -z \"\$2\" ] && { echo 'Usage: wanxiang-host shell <command>' >&2; exit 1; }",
            "    shift; CMD=\"\$*\"",
            ...
            "    echo \"  wanxiang-host health               Bridge health and ADB status\"",
            "    exit 1;;",
            "esac",
        ).joinToString("\n") + "\n"
        ...
        val WANXIANG_ANDROID_EXEC_SCRIPT = listOf(
            ...
            "",
            "[ -z \"\$1\" ] && { echo 'Usage: wanxiang-android-exec <binary> [args...]' >&2; exit 1; }",
