...
class OperationCoordinatorTest {
    ...
    private lateinit var eventBus: top.wanxiang.app.harness.events.HarnessEventBus
    private lateinit var coordinator: OperationCoordinator

    @Before
    fun setUp() {
        repository = FakeRuntimeRepository()
        eventBus = top.wanxiang.app.harness.events.HarnessEventBus()
    ...
    fun `operation lifecycle emits structured events`() = runBlocking {
        ...
        try {
            ...
            val finished = events.filterIsInstance<top.wanxiang.app.harness.events.HarnessEvent.OperationFinished>().single()
