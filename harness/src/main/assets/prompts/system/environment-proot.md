...
- ADB 守护进程：PRoot 沙箱内无持久 init/systemd，后台 adb daemon (fork-server) 每次调用后都会随子进程退出而重启；命令行 ADB 操作需在同一命令内完成 connect + 目标操作（/opt/wanxiang/bin/adb 封装脚本已内置自动 connect），或直接使用 logcat-grabber / host(action="logcat") 经由宿主持久桥接。
