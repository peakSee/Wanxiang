...
- 依赖优先执行 `flutter pub get`；构建入口：`/opt/wanxiang/scripts/build_flutter.sh "{{WORKSPACE_PATH}}" "apk --debug"`，或 `flutter build apk --debug`。
...
- 安装到手机时，确认 `build/app/outputs/flutter-apk/*.apk` 完整后复制到 `/sdcard/Download/`，再执行 `wanxiang-host install-apk <apk路径>`；检测到 ADB 后可直接 `adb install -r <apk>`。排查运行日志或崩溃时直接调用 `host(action="logcat", package="<包名>")` 或执行 `logcat-grabber <包名>`，无需 Shizuku/Root。
