...
import top.wanxiang.app.core.database.HarnessLaneResultEntity
import top.wanxiang.app.core.database.HarnessOperationEntity
import top.wanxiang.app.core.database.HarnessRuntimeRepository
import top.wanxiang.app.core.database.HarnessUsageEntity
import top.wanxiang.app.harness.ChatUsage
...
class OperationCoordinator @Inject constructor(
    ...
    suspend fun toolIntent(operationId: String, message: HarnessMessage, payloadJson: String, replay: ReplayPolicy, round: Int) {
        ...
        emitFor(operationId) { sessionId, timestamp, _ ->
            val toolCall = message as? top.wanxiang.app.harness.ToolCall
    ...
    suspend fun toolSettled(operationId: String, message: HarnessMessage, round: Int, toolName: String? = null) {
        ...
        emitFor(operationId) { sessionId, timestamp, _ ->
            val result = message as? top.wanxiang.app.harness.ToolResult
...
private fun HarnessMessage.serialType(): String = when (this) {
    is top.wanxiang.app.harness.UserMessage -> "user"
    is top.wanxiang.app.harness.AssistantText -> "assistant"
    is top.wanxiang.app.harness.ToolCall -> "tool_call"
    is top.wanxiang.app.harness.ToolResult -> "tool_result"
    is top.wanxiang.app.harness.CapabilityEvent -> "capability_event"
}
