...
internal object RtkCommandOptimizer {
    private const val RTK_BINARY = "/opt/wanxiang/bin/rtk"

    private val rtkEnvironment = mapOf(
        // Raw failure output can include project secrets and is already returned to the Agent.
        "RTK_TEE" to "0",
        "XDG_CONFIG_HOME" to "/opt/wanxiang/data/rtk/config",
        "XDG_DATA_HOME" to "/opt/wanxiang/data/rtk/data",
    )
    ...
    private fun wrapWithFallback(command: String): String {
        ...
        return """
            if [ -x "$RTK_BINARY" ]; then
                _wanxiang_rtk_rewritten="${'$'}("$RTK_BINARY" rewrite $quotedCommand 2>/dev/null)"
                _wanxiang_rtk_status=${'$'}?
                case "${'$'}_wanxiang_rtk_rewritten" in
                    "rtk "*) ;;
                    *) _wanxiang_rtk_status=1 ;;
                esac
                if [ "${'$'}_wanxiang_rtk_status" -eq 0 ] && [ "${'$'}(printf '%s' "${'$'}_wanxiang_rtk_rewritten" | wc -l)" -eq 0 ]; then
                    eval "${'$'}_wanxiang_rtk_rewritten"
                else
                    $command
                fi
            else
