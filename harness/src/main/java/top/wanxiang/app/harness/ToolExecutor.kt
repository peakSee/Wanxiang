...
import top.wanxiang.app.runtime.privilege.BinderOutcome
import top.wanxiang.app.runtime.privilege.PrivilegeManager
import top.wanxiang.app.runtime.privilege.ShizukuSystemApis
...
class ToolExecutor @Inject constructor(
    ...
    private val approvalRepository: top.wanxiang.app.core.database.AgentApprovalRepository? = null,
    private val sessionDao: HarnessSessionRepository? = null,
    private val subagentOrchestrator: SubagentOrchestrator? = null,
    private val mcpManager: top.wanxiang.app.harness.mcp.McpManager? = null,
    private val contextExecutor: AgentContextExecutor? = null,
    private val messageStore: SessionTreeStore? = null,
    private val eventBus: top.wanxiang.app.harness.events.HarnessEventBus? = null,
    ...
    private val hostGuiController: top.wanxiang.app.runtime.gui.HostGuiController? = null,
    private val buildScriptToolExecutor: BuildScriptToolExecutor? = null,
    private val promptRouter: top.wanxiang.app.harness.prompt.PromptRouter? = null,
    private val checkpointStore: top.wanxiang.app.harness.checkpoint.CheckpointStore? = null,
    private val dualAgentCoordinator: top.wanxiang.app.harness.dual.DualAgentCoordinator? = null,
        ...
        val outcome = try {
            if (!bypassApproval && sessionId.isNotBlank()) {
                ...
                if (decision.required) {
                    ...
                    eventBus?.emit(
                        top.wanxiang.app.harness.events.HarnessEvent.ApprovalRequested(
                            sessionId = sessionId,
                            timestamp = now,
    ...
    private suspend fun executeTool(
        ...
        val args = top.wanxiang.app.harness.validation.ToolSchemaValidator.normalizeArgs(rawArgs)
    ...
    private suspend fun executeHost(args: JsonObject, operationId: String?, sessionId: String): Pair<Boolean, String> {
        ...
        if (action == "settings_put") {
            ...
            if (namespace == "system") {
                ...
                eventBus?.emit(
                    top.wanxiang.app.harness.events.HarnessEvent.PermissionRequired(
